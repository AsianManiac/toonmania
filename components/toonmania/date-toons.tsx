...
export const GenreHead = ({ genre, text }: GenreHeadProps) => {
  return (
    <div
      className={`lg:w-48 w-36 lg:h-48 h-36 hm-${makeURLFriendly(
        genre
      )} cursor-pointer`}
    >
...
const Datetoons: React.FC<HomeToonsProps> = ({ ...props }) => {
  const { image, title, author, likes, genre, shortSummary, longSummary } =
    props;
  return (
    <>
        ...
        <Card className="lg:w-48 w-36 rounded-none group opacity-100 group-hover:opacity-0 transition-opacity duration-200 relative cursor-pointer">
          <CardContent className="relative p-0">
            ...
            <div className="absolute lg:top-[14px] top-2 px-4">
              <CardTitle className="font-medium text-sm lg:text-[15px] pb-1">
                {title}
              </CardTitle>
              <p className="text-sm flex items-center">
                <span>
                  <HeartIcon
                    size={"16"}
                    fill="#00dc64"
                    className="text-[#00dc64]"
                  />
                </span>
                <span className="text-[#00dc64] font-medium lg:text-xs text-[10px]">
                  {" "}
                  {likes}{" "}
                </span>
              </p>
            ...
            <div className="absolute bottom-5 px-4">
              <p className={`text-xs font-medium ${makeURLFriendly(genre)}`}>
                {genre}
              </p>
            </div>
          </CardContent>
          <div
            className={`absolute top-0 left-0 w-full h-full bg-${makeURLFriendly(
              genre
