...
interface EpisodeNumberFormProps {
  initialData: { episodeNumber: any };
  toonId: string;
  episodeId: string;
}
...
export const EpisodeNumberForm = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.success("Episode Number successfully");
      toggleEdit();
      router.refresh();
    } catch (error: any) {
  ...
  return (
    <div className="mt-6 bg-slate-200 rounded-md p-4">
      ...
      {!isEditing && (
        <p className="text-sm mt-2">#{initialData.episodeNumber}</p>
      )}
      {isEditing && (
        <Form {...form}>
          <form
