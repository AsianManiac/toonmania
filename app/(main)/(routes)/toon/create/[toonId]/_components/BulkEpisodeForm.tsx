...
import { Attachments, Webtoon } from "@prisma/client";

interface BulkEpisodeFormProps {
  initialData: Webtoon & { attachments: Attachments[] };
  toonId: string;
}
...
export const BulkEpisodeForm = ({
  initialData,
  toonId,
}: BulkEpisodeFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.post(`/api/v1/toons/${toonId}/attachments`, values);
  ...
  const onDelete = async (id: string) => {
    try {
      setdeletingId(id);
      await axios.delete(`/api/v1/toons/${toonId}/attachments/${id}`);
  ...
  return (
    <div className="mt-6 bg-slate-200 rounded-md p-4">
      <div className="font-medium flex items-center justify-between">
        Toon Soundtracks
        <Button onClick={toggleEdit} disabled={true} variant={"ghost"}>
          {isEditing ? (
            <>Cancel</>
          ) : (
            <>
